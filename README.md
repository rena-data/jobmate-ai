...
채용 공고 URL을 입력하거나 **키워드로 채용 플랫폼을 검색해 신규 공고를 자동 수집**하고, 공고 내용을 자동으로 정리해서 Google Sheets에 저장해주며, 마감 2일 전·지원 후 후속이 없을 때 Slack으로 알려주는 도구입니다.
...
**방법 2 — 키워드 자동 수집 (신규)**
```
1. 키워드(예: AX, AI Engineer)를 등록합니다
2. 원티드·사람인·잡코리아를 검색해 신규 공고 URL을 찾습니다
3. 이미 수집한 공고(같은 URL 또는 같은 회사+직무)는 중복 제거합니다
4. 새 공고만 자동으로 Google Sheets에 저장합니다 (수동/예약 실행)
```

> 두 방법 모두 마감 2일 전, 지원 후 7일째 후속이 없으면 Slack으로 알림이 옵니다.
...
> **URL 직접 입력은 "개별 공고" 주소만 지원합니다.** 목록/검색 페이지(예: 원티드 `/wdlist/...`, 사람인 `/jobs/list/...`, 그룹바이 `/positions`)가 아니라, 공고 하나를 클릭했을 때 나오는 주소(예: 원티드 `/wd/258066`, 그룹바이 `/positions/10760`)를 넣어주세요.
> (**키워드 자동 수집**은 검색 페이지를 자동으로 훑어 개별 공고를 찾으므로 이 제약과 무관합니다 — 아래 "키워드 자동 수집" 참고.)
...
| **채용공고 › 수집** | URL을 붙여넣고 `수집` → 미리보기 확인·수정 → `저장`. 하단에 **🔍 키워드 자동 수집** 섹션(키워드 입력 → 검색 → 신규만 자동 저장 + 키워드 저장) |
| **채용공고 › 목록** | 저장된 공고 보기 (로컬 캐시 / Google Sheets). 행 클릭 시 **상세 보기 + 상태·메모 편집**. 수집방식/검색키워드 컬럼 표시, 로컬 캐시 소스에선 **캐시 비우기** 가능 |
| **알림·관리 › 마감 임박** | 마감 임박 공고 조회 → `Slack으로 발송` (발송 이력과 무관하게 **다시 발송 가능**) |
| **알림·관리 › 지원상태 변경** | 공고 선택 → 상태 변경 (관심공고/지원완료/서류탈락/서류합격/면접예정/면접탈락/최종합격/불합격/보류 — 9단계) |
...
| 우대사항 | 우대 조건 |
| 마감일 | 지원 마감 날짜 |
| 상태 | interest(관심공고) / applied(지원완료) / document_fail(서류탈락) / document_pass(서류합격) / interview(면접예정) / interview_fail(면접탈락) / final_pass(최종합격) / rejected(불합격) / hold(보류) |
| 비고 | 개인 메모 (상세 보기에서 편집) |

> **자동 수집한 공고**는 끝열에 `검색키워드 · 수집플랫폼 · 신규여부 · 수집방식(자동/수동)` 컬럼이 추가로 기록됩니다(수동 수집 행은 빈칸 → "수동"으로 표시). 15개 기본 컬럼 순서는 그대로 유지됩니다.
...
- ✅ 키워드 기반 자동 수집 (원티드·사람인·잡코리아 검색 → URL+회사·직무 2단계 중복 제거 → 자동 저장; 웹 UI·CLI·cron, 지원상태 9단계, 마감 알림 재발송) — 완료
- 키워드 자동 수집 플랫폼 확대 (그룹바이·점핏)
- 신규 공고 발견 시 Slack 알림 / 키워드·플랫폼 통계 대시보드
